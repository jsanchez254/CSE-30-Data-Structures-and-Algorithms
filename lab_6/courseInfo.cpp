...
void courseInfo(struct Course info,string &endTime, string &startTime, int j);	//will print out 
...
int main()
{
    ...
    string endTime;					//will store end time			
	...
	cout<<"\n---------------------"<<endl;
    	z++;
    	courseInfo(courseInf,endTime,startTime, j);

    	if(getTimeFromUser(start,startTime))		//checks if starting time is correct
    	{

        	if(getTimeFromUser(end, endTime))	//checks if ending time is correct
        	{
...
//COURSE INFO
void courseInfo(struct Course info,string &endTime, string &startTime, int j)
{
	...
	file>>dummy;			//reads the number of courses everytime file has to be read
		...
		while(file >> info.name>> info.credits>>info.majorRequirement
        	     >>info.avgGrade >>info.days>>startTime>>endTime)
		{
			i++;
